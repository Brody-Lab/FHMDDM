			...
			if θ.fit_Δ𝐯
				for k in eachindex(θ.Δ𝐯)
					for q in eachindex(θ.Δ𝐯[k])
						counter += 1
						concatenatedθ[counter] = θ.Δ𝐯[k][q]
						indexθ[i][n].Δ𝐯[k][q] = offset + counter
					end
				end
			end
		end
...
OPTIONAL ARGUMENT
-`intialization`: whether only the parameters included in the initialization are counted (thereby excluding `b` and `Δ𝐯`)
...
function sortparameters!(θ::GLMθ, concatenatedθ::Vector{<:Real}; offset::Integer=0, initialization::Bool=false)
	...
	if θ.fit_Δ𝐯
		for k in eachindex(θ.Δ𝐯)
			for q in eachindex(θ.Δ𝐯[k])
				counter+=1
				θ.Δ𝐯[k][q] = concatenatedθ[counter]
			end
		end
	end
	return nothing
...
function sortparameters!(θ::GLMθ, concatenatedθ::Vector{<:Real}, index::GLMθ)
	...
	if θ.fit_Δ𝐯
		for k in eachindex(θ.Δ𝐯)
			for q in eachindex(θ.Δ𝐯[k])
				θ.Δ𝐯[k][q] = concatenatedθ[index.Δ𝐯[k][q]]
			end
		end
	end
	return nothing
...
function sortparameters!(θall::Vector{<:Real}, index::GLMθ, θglm::Vector{<:Real})
	...
	if fit_Δ𝐯
		for k in eachindex(θ.Δ𝐯)
			for q in eachindex(index.Δ𝐯[k])
				counter+=1
				θall[index.Δ𝐯[k][q]] = θglm[counter]
			end
		end
	end
	return nothing
