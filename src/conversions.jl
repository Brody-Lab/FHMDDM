...
function dictionary(options::Options)
	Dict(	"a_latency_s"=>options.a_latency_s,
			"atbf_begins0"=>options.atbf_begins0,
			"atbf_constantfunction"=>options.atbf_constantfunction,
			"atbf_ends0"=>options.atbf_ends0,
			"atbf_hz"=>options.atbf_hz,
			"atbf_period"=>options.atbf_period,
			"atbf_stretch"=>options.atbf_stretch,
			"alpha0_choices"=>options.α₀_choices,
			"alpharangeDDM"=>options.αrangeDDM,
			"alpharangeGLM"=>options.αrangeGLM,
...
function Options(options::Dict)
	Options(a_latency_s = options["a_latency_s"],
			...
			αrangeDDM=vec(options["alpharangeDDM"]),
			αrangeGLM=vec(options["alpharangeGLM"]),
			choiceobjective=options["choiceobjective"],
			datapath = options["datapath"],
			Δt = options["dt"],
