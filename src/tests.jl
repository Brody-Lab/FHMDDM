...
function test(datapath::String; maxabsdiff::Real=1e-8)
	...
	maximizelikelihood!(Model(datapath), Optim.LBFGS(linesearch = LineSearches.BackTracking()); iterations=50)
	printseparator()
	...
	folderpath = joinpath(dirname(model.options.datapath), "test")
	save(ModelSummary(model), folderpath)
	printseparator()
	println("loading model parameters from saved summary")
	model = Model(datapath)
	folderpath = joinpath(dirname(model.options.datapath), "test")
	filepath = joinpath(folderpath, "modelsummary.mat")
	FHMDDM.sortparameters!(model, filepath)
	printseparator()
	println("saving model characterization")
	folderpath = joinpath(dirname(model.options.datapath), "test")
	save(Characterization(model), folderpath)
	printseparator()
	println("testing cross-validation and saving results")
	model = Model(datapath)
	folderpath = joinpath(dirname(model.options.datapath), "cvtest")
