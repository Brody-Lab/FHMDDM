...
function test(datapath::String; maxabsdiff::Real=1e-8)
	...
	printseparator()
	println("loading model parameters from from `test.mat`")
	Model(datapath; prefix="test")
	printseparator()
	...
	model = Model(datapath; prefix="")
	samplepaths = samples(model, 2)
	printseparator()
	println("loading a sample")
	sample1 = Model(samplepaths[1])
	printseparator()
